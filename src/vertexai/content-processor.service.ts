...
import { StructuredContent, ContentSection, CodeSnippet } from './dto/structured-response.dto';
...
export class ContentProcessorService {
  ...
  async processContent(
    ...
    try {
      ...
      const sections = this.extractSections(rawText, contentType, includeExamples);
  ...
  private extractSections(text: string, contentType: string, includeExamples?: boolean): ContentSection[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (this.isHeaderLine(line)) {
        ...
        if (currentSection) {
          ...
          currentSection.keyPoints = this.extractKeyPointsFromSection(currentContent);
  ...
  private extractKeyTakeaways(text: string): string[] {
    ...
    if (bulletPoints) {
      keyTakeaways.push(...bulletPoints.map(point => point.replace(/^[\-\*\+]\s+/, '')));
    }
  ...
  private extractKeyPointsFromSection(text: string): string[] {
    ...
    if (listItems) {
      points.push(...listItems.map(item => 
  ...
  private estimateDifficulty(text: string): 'beginner' | 'intermediate' | 'advanced' {
    const complexWords = text.match(/\b\w{10,}\b/g)?.length || 0;
    const technicalTerms = text.match(/\b(algoritmo|implementación|arquitectura|optimización|refactoring|debugging)\b/gi)?.length || 0;
